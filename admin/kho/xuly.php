		...
		switch ($action) {
			case 'cong':
				...
				if (isset($rs)) {
					...
					header('location:' . $_SERVER["HTTP_REFERER"] . '&tb=loi'); 
				}
				break;
			...
			case 'tru':
				foreach ($chon as $key => $value) {
					...
					$sql2 = "INSERT INTO `phieuxuat`(`MaNV`, `MaSP`, `MaMau`, `Size`, `SoLuong`, `DonGia`, `TongTien`, `Note`) VALUES ($manv, $masp, '$mau', '$value', $so, '$dg', '$tt', '$note')";
					$rs2 = mysqli_query($conn, $sql2);
				}
